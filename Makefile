...
clean:
	rm -f $(OBJECTS) $(DEPENDS) marklinctl.elf marklinctl.img

marklinctl.img: marklinctl.elf
	$(OBJCOPY) $< -O binary $@

marklinctl.elf: $(OBJECTS) linker.ld
	$(CC) $(CFLAGS) $(filter-out %.ld, $^) -o $@ $(LDFLAGS)
	@$(OBJDUMP) -d marklinctl.elf | fgrep -q q0 && printf "\n***** WARNING: SIMD INSTRUCTIONS DETECTED! *****\n\n" || true
